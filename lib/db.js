...
//  console.log('In the db module :',page);
  getDb(function(err, db) {
    if (err) {
      return done(err);
    }
    ...
    db.collection('uploads')
      ...
      .toArray(function(err, items) { // eslint-disable-line no-shadow
        //console.log('Items return by uploads collections :',items.length);
        if (err) {
          return done(err);
        }
      //  console.log(limit);
        var more = !!items[limit];
      //console.log('more in Db :'+more);

        if (more) {
          var modArray = items.splice(items.length - 1);
        }
        // console.log("here");

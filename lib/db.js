...
var debug = require('debug')('clipboard: db');
var hash = new Hashids('', 5);
var limit = 15; // actual 20 -> 19
var skip = 0;
...
exports.fetchItems = function(page, done) {
  ...
  getDb(function(err, db) {
    ...
    db.collection('uploads')
      .find(null, null, {
        limit: limit + 1,
        skip: page * limit,  // skip skips the first skip no of documents and retreive
          sort: [           //  the documents after the skip value
          ['createdms', 'desc']
        ]
      })
      .toArray(function(err, items) { // eslint-disable-line no-shadow
        //debug('Items return by uploads collections :', items.length);
        if (err) {
          return done(err);
        }

        if(!items){
          return done(new Error('No such file present'));
        }

        var more = !!items[limit];
        debug('more in Db :' + more);

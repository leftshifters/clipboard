...
function reindex(items, done) {
  ...
  es.ping({
    ...
    if (error) {
      console.error('ERROR!');
      console.trace(error);
      done(new Error('Elasticsearch is not responding'));
    } else {
...
function search(page, query, done) {
  ...
  function onSearch(err, response) {
    ...
    //var more = response.hits.total - (response.hits.hits.length + (size)) > 0;
    var more = (response.hits.hits.length - size ) > 0;
    debug('More in search module :' +more);
  ...
  /*
  Search method of elasticsearch client :-
  Test Cases : Returns 'size' no of matched documents starting from (page*size +1) documents.
  */
